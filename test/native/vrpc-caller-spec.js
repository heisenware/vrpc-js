...
describe('Embedding C++ to Node.js', () => {
  context('An instance of the VrpcCaller class', () => {
    ...
    it('should be construct-able given a c++ native addon', () => {
      caller = new VrpcCaller(addon)
      assert.ok(caller)
    })
    it('should show all available meta data', () => {
      const actual = caller.getMetaData('TestClass')
    ...
    context('The corresponding VrpcCaller instance', () => {
      ...
      context('The corresponding TestClass proxy', () => {
        ...
        it('should be able to call a static function', () => {
          assert.equal(caller.callStatic('TestClass', 'crazy'), 'who is crazy?')
        })
        it('and overloads thereof', () => {
          assert.equal(caller.callStatic('TestClass', 'crazy', 'vrpc'), 'vrpc is crazy!')
        })
        it('should create a second instance', async () => {
          anotherTestClass = caller.create({
            className: 'TestClass',
            args: [{
              testEntry: [{
  ...
  context('Two instances of the VrpcCaller class', () => {
    let caller1
    let caller2
    const emitter = new EventEmitter()
    const newEntries = []
    ...
    it('should be construct-able given the same c++ native addon', () => {
      caller1 = new VrpcCaller(addon)
      caller2 = new VrpcCaller(addon)
      assert.ok(caller1)
      assert.ok(caller2)
    ...
    context('The corresponding VrpcCaller instance', () => {
      ...
      it('should be able to create a TestClass proxy using default constructor', () => {
        testClass1 = caller1.create({ className: 'TestClass' })
        testClass2 = caller2.create({ className: 'TestClass' })
      ...
      context('The corresponding TestClass proxies', () => {
        ...
        it('should be able to call a static function', () => {
          assert.equal(caller1.callStatic('TestClass', 'crazy'), 'who is crazy?')
          assert.equal(caller2.callStatic('TestClass', 'crazy'), 'who is crazy?')
        })
        it('and overloads thereof', () => {
          assert.equal(caller1.callStatic('TestClass', 'crazy', 'vrpc'), 'vrpc is crazy!')
          assert.equal(caller2.callStatic('TestClass', 'crazy', 'vrpc'), 'vrpc is crazy!')
        })
        it('should create a second instance', async () => {
          anotherTestClass1 = caller1.create(
            'TestClass',
            {
              testEntry: [{
          ...
          anotherTestClass2 = caller2.create(
            'TestClass',
            {
              testEntry: [{
