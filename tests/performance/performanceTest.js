...
describe('A single client connected to a single agent', () => {
  ...
  it('should create multiple proxy instances', async () => {
    ...
    for (let i = 0; i < N_PROXIES; i++) {
      ...
      const proxy = await this.client.create({
        agent: 'agent1',
        className: 'PlusOne'
      })
    ...
    console.log(
      'Average time for proxy-creation',
      this.ave.reduce((a, b) => a + b) / this.ave.length
    )
    this.ave = []
  })
  ...
  it('should not loose messages when calling the agent full speed', async () => {
    for (let i = 1; i <= N_RPC_CALLS; i++) {
      ...
      const values = await Promise.all(
        this.proxies.map(proxy => proxy.increment())
      )
      performance.mark(`${i}e`)
  ...
  it.skip('should not even loose messages when the agent is shortly offline', async () => {
